 ...
 *
 * Scala 3 outcome (`sbt "run 5000000 1"`): `java.lang.StackOverflowError` inside `mirror` on the very
...
@main def main(treeDepth: Int, mirrorCount: Int): Unit = println(runDemo(treeDepth, mirrorCount))

def runDemo(treeDepth: Int, mirrorCount: Int): Int = deepestLeftA(0, mirrorN(mirrorCount, buildTree(treeDepth)))
...
def mirrorN[A](times: Int, t: Tree[A]): Tree[A] =
  if times == 0 then t
  else mirrorN(times - 1, mirror(t)) // tail recursion
