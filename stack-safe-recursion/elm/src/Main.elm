...
read from argv (passed as Elm flags by the Node runner); the recorded run
(assert-behavior.sh) passes `5000000 1`.

Elm outcome (`npm start -- 5000000 1`): `RangeError: Maximum call stack size exceeded` inside `mirror` on
...
calls, so the depth-5000000 V-tree exhausts the JS engine's call stack before
...
type alias Flags =
    { treeDepth : Int, mirrorCount : Int }


main : Program Flags () ()
main =
    Platform.worker
        { init = \flags -> ( (), stdout (String.fromInt (runDemo flags.treeDepth flags.mirrorCount)) )
