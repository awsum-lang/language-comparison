#!/bin/sh
# Expected: compiles fine, then crashes at runtime — Elm emits a loop only for
...
status=0
# --silent suppresses npm's "> ... node runner.js 5000000 1" preamble, which
# would otherwise echo the arg into stdout and trip the result-absence check.
