...
# JVM
actual=$(awsum run --program-type cli -t jvm Main.aww -- 5000000 1)
if [ "$actual" != "5000000" ]; then
  printf 'FAIL: target=jvm\nexpected: 5000000\nactual:   %s\n' "$actual"
  exit 1
fi
echo "OK: jvm prints 5000000"

# CLR
actual=$(awsum run --program-type cli -t clr Main.aww -- 5000000 1)
if [ "$actual" != "5000000" ]; then
  printf 'FAIL: target=clr\nexpected: 5000000\nactual:   %s\n' "$actual"
  exit 1
fi
echo "OK: clr prints 5000000"

# WASM — bounded by the ~4 GB wasm32 linear-memory cap
actual=$(awsum run --program-type cli -t wasm Main.aww -- 5000000 1)
if [ "$actual" != "5000000" ]; then
  printf 'FAIL: target=wasm\nexpected: 5000000\nactual:   %s\n' "$actual"
  exit 1
fi
echo "OK: wasm prints 5000000"

# JS — bounded by the V8 heap
actual=$(awsum run --program-type cli -t js Main.aww -- 5000000 1)
if [ "$actual" != "5000000" ]; then
  printf 'FAIL: target=js\nexpected: 5000000\nactual:   %s\n' "$actual"
  exit 1
fi
echo "OK: js prints 5000000"
