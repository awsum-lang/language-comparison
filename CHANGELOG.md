...
- Each language directory in `recursion-and-memory-safety/` carries an `assert-behavior.sh` asserting its recorded outcome — Awsum and Haskell print `500000`, the other ten crash. `just test` runs them all, `just test <language>` runs one.
...
- The scenario runs at tree depth 500_000 (was 100_000) — deep enough that stack-budget and JIT-tier luck stop changing outcomes: Rust aborts on Linux runners too, C# dies already in `BuildLeft`, and F#'s Release tier-up race is gone (it aborts inside `mirror` in every configuration). The Haskell entry drops its perf figures — performance is not the topic, safety is.
